...
class RDKit_desc(Scorer):
    def __init__(self, rdkit_descriptors=None, compute_3Drdkit=False):
        self.descriptors = rdkit_descriptors if rdkit_descriptors else Descriptors._descList
        if compute_3Drdkit:
            self.descriptors = self.descriptors + ['Asphericity', 'Eccentricity', 'InertialShapeFactor',
    ...
    def getScores(self, mols):
        ...
        calc = MoleculeDescriptors.MolecularDescriptorCalculator([x[0] for x in self.descriptors])
