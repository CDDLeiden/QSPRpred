...
class ProteinDescriptorCalculator(DescriptorsCalculator):
    ...
    Attributes:
        descsets (list[ProteinDescriptorSet])
            List of associated protein descriptor sets.
        msaProvider:
            Multiple sequence alignment provider for descriptors that require it.
    """
    
    def __init__(
            self,
            descsets: list[ProteinDescriptorSet],
            msa_provider=ClustalMSA()
    ) -> None:
        """Initializes the ProteinDescriptorCalculator.
