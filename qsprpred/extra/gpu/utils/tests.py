...
from qsprpred.utils.parallel import ThreadsJITGenerator, batched_generator
...
class TestThreadedGeneratorsGPU(QSPRTestCase):
    ...
    def testSimpleGPU(self):
        ...
        p_generator = ThreadsJITGenerator(
            len(self.GPUs), use_gpus=self.GPUs, worker_type="gpu", jobs_per_gpu=2
        )
        self.assertListEqual(
            [x**2 for x in range(10)],
            sorted(p_generator(
    ...
    def testBatchedGPU(self):
        generator = batched_generator(range(10), 2)
        p_generator = ThreadsJITGenerator(
            len(self.GPUs), use_gpus=self.GPUs, worker_type="gpu"
        )
        self.assertListEqual(
            [[0, 1], [4, 9], [16, 25], [36, 49], [64, 81]],
            sorted(p_generator(generator, self.gpu_func_batched)),
        )
